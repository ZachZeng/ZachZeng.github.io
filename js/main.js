...
;(function(window) {
	...
	IsoGrid.prototype._createPseudoScroller = function() {
		...
		this.pseudoScrollerEl.style.height = this.gridEl.offsetHeight + 300 + getComputedTranslateY(this.isolayerEl) * Math.sqrt(2) + 'px';	
	};
	...
	IsoGrid.prototype._initEvents = function() {
		...
		if( this.options.type === 'scrollable' ) {
			...
			this.msnry.on('layoutComplete', function( laidOutItems ) {
				...
				self.pseudoScrollerEl.style.height = self.gridEl.offsetHeight + 200 + self.isolayerEl.offsetTop * Math.sqrt(2) + 'px';
				self._scrollPage();
			});
		}
